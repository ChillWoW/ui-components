...
const ComponentCard = () => {
    ...
    const renderInfoPanel = () => {
        ...
        return (
            <div className="h-full flex items-center justify-center">
                <Text>
                    No reference information available for this component
                </Text>
            </div>
        );
    };
    ...
    const generateComponentCode = () => {
        ...
        const propEntries = Object.entries(currentProps).filter(
            ([key, value]) => {
            ...
            if (Array.isArray(componentProps.children)) {
                componentProps.children.forEach((child: any) => {
                    if (React.isValidElement(child)) {
                        result += `    ${getComponentString(child).replace(
                            /^/gm,
                            "  "
                        )}\n`;
                    } else if (child) {
        ...
        <div className="bg-dark-900 min-h-screen w-full flex flex-col items-center p-8 gap-8">
            {mobileAlert && (
                <div className="w-full max-w-6xl">
                    ...
                    >
                        <Alert.Title>
                            Website Mobile Support Coming Soon!
                        </Alert.Title>
                        <Alert.Description>
                            This website doesn't support mobile devices at the
                            moment. We're working on it, and it'll be fixed in
                            the next update
                        </Alert.Description>
                    </Alert>
                </div>
            ...
            <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Card
                    className="bg-dark-800 border-dark-600 md:col-span-2"
                    withBorder
                >
                    <Text
                        size="lg"
